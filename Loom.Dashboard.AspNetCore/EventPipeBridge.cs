...
public sealed class EventPipeBridge : BackgroundService
{
    ...
    private async Task StreamMetrics(CancellationToken ct)
    {
        ...
        source.Dynamic.All += traceEvent =>
        {
            ...
            // accumulator is the wrong home for it) and reads "value", NOT "rate": a
            // gauge is exported as its newest sample, and once a level stops moving it
            // publishes rate=0 every interval while value holds (measured: rate=0
            // value=10 against a pool held at 10). Reading "rate" would report a steady
            // pool of 10 connections as 0.
            //
            // Two measured limitations of the runtime, NOT of this code, that no choice
            // of field can fix. Both were observed directly:
            //   - "value" is cumulative WITHIN THE SESSION, not since the instrument was
            //     created. A session attaching after the level settles sees the change
            //     since it attached, not the absolute level.
            //   - an up-down counter with no activity during a session is not reported
            //     by that session at all: attaching 15s after the fixture reached its
            //     plateau ingested zero records for it.
            // The bridge runs for the life of the dashboard, so the common case tracks
