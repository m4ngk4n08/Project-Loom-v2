...
// Static, allocation-free response headers (CSP, X-Frame-Options, etc). Placed at the
// front of the pipeline - before authentication and before UseStaticFiles below - so a
// short-circuited response still carries them (BACKLOG.md 4.7). Moved into
// Loom.Dashboard.AspNetCore so a host embedding the library gets the same protection
// without copying this block itself (BACKLOG.md § 6.29); see
// EndpointExtensions.UseLoomDashboardSecurityHeaders for the CSP reasoning.
