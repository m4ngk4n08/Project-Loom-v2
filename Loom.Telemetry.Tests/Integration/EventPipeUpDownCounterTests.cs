...
/// up-down counter is still ramping. Two runtime behaviours make a late attach useless
/// here, both measured directly rather than assumed:
...
///     zero records for it.
/// Sharing EventPipeHarnessTests' fixture would attach after the ramp finished and test
/// nothing, which is exactly how this started out.
/// </summary>
[Trait("Category", "Integration")]
public sealed class EventPipeUpDownCounterTests : IClassFixture<FixtureProcess>
{
    private readonly FixtureProcess _fixture;

    public EventPipeUpDownCounterTests(FixtureProcess fixture)
    {
        _fixture = fixture;
    }
    ...
    public async Task UpDownCounter_ReadsLevel_NotIntervalDelta()
    {
        ...
        try
        {
            ...
            // mid-ramp and passes under both fields. Verified: an earlier version of
            // this test did exactly that and passed with "rate". Only after the level
            // settles do the two diverge, with rate falling to 0 while value holds.
            await Task.Delay(TimeSpan.FromSeconds(8));

            var records = store.ReadRecent("fixture.active.connections", 50);

            Assert.True(records.Length > 0,
                $"No fixture.active.connections records ingested at all. IsFaulted={collector.IsFaulted}, " +
